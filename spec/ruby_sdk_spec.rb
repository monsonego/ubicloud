...
RSpec.describe Ubicloud do
  ...
  it "Postgres#add_log_destination and #delete_log_destination work without log destinations loaded" do
    expect(Clover).to receive(:call).twice.and_invoke(proc do |env|
      [200, {"content-type" => "application/json"}, ["{}"]]
    end)
    ...
    pg.add_log_destination(name: "my-dest", host: "logs.example.com", port: 6514)
    expect(pg.values[:log_destinations]).to be_nil
    pg.delete_log_destination("p1345678901234567890123456")
    expect(pg.values[:log_destinations]).to be_nil
  end

  it "Postgres#add_log_destination and #delete_log_destination modify log destinations if loaded" do
    expect(Clover).to receive(:call).twice.and_invoke(proc do |env|
