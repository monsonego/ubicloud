...
RSpec.describe PostgresServer do
  ...
  describe "#logs_config" do
    it "returns config with resource_id, instance, server_role, log_dir, and empty destinations" do
      ...
      expect(config[:log_dir]).to eq("/dat/16/data/pg_log")
    ...
    it "includes log destinations" do
      ...
      expect(destinations.first).to eq({host: "logs.example.com", port: 6514, application_name: resource.name, structured_data: nil})
    end
  end
end
